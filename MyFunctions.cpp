...
// This code is modefied between line number 2471 and 2657.
...

{
    ...
    vector<long double> temp_cage_com;
    double dx, dy, dz, boxX, boxY, boxZ, dist;
    int fact = 10; // Converting nanometer to angstrom
    
    out5 = rawFilename + "_cage512";
    out6 = rawFilename + "_cage62512";
    out64512 = rawFilename + "_cage64512";
                ...
                if( N_rings == 28 && m <=3  )  //This part is for base ring of 6(2)5(12) cups which are 6-ring are indexed 0,1,2,3.
                {
                    ...
                    for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
                    {
                        ...
                        {                               //If atoms is not present in atoms_set, count will be zero, condition is one and
                            ...
                            outFile << setprecision(3);
                            outFile << setw(5) << "OW" ;
                            outFile << setw(8) << atom_Pos[atoms][0] * fact;
                            outFile << setw(8) << atom_Pos[atoms][1] * fact;
                            outFile << setw(8) << atom_Pos[atoms][2] * fact;
                            outFile << "\n";
                            atom_counter++;
                            
                            outFile << setw(5) << "HW1" ;
                            outFile << setw(8) << atom_Pos[atoms+1][0] * fact;
                            outFile << setw(8) << atom_Pos[atoms+1][1] * fact;
                            outFile << setw(8) << atom_Pos[atoms+1][2] * fact;
                            outFile << "\n";
                            atom_counter++;
                            
                            outFile << setw(5) << "HW2" ;
                            outFile << setw(8) << atom_Pos[atoms+2][0] * fact;
                            outFile << setw(8) << atom_Pos[atoms+2][1] * fact;
                            outFile << setw(8) << atom_Pos[atoms+2][2] * fact;
                            outFile << "\n";
                            atom_counter++;
                            
                            outFile << setw(5)  << "MW" ;
                            outFile << setw(8) << atom_Pos[atoms+3][0] * fact;
                            outFile << setw(8) << atom_Pos[atoms+3][1] * fact;
                            outFile << setw(8) << atom_Pos[atoms+3][2] * fact;
                            outFile << "\n";
                            atom_counter++;
    ...
    // molecule_id map will store the molecule name we wish to print in the output files. 
    ...
    // molecule_atoms map will store the number of atoms (coordinates given in input file for each molecule).
    map<string, int> molecule_atoms;
    molecule_atoms["CO2"]=3;
    molecule_atoms["Met"]=1;
    molecule_atoms["METH"]=6;
    molecule_atoms["NH3"]=4;
    
    
    // m_atm will store the atom name we wish to print in the output file.
    map<string, vector<string>> m_atm;
    m_atm["CO2"].push_back("C1");
    m_atm["CO2"].push_back("O2");
    m_atm["CO2"].push_back("O3");
    m_atm["Met"].push_back("CM");
    m_atm["METH"].push_back("O1");
    m_atm["METH"].push_back("C2");
    m_atm["METH"].push_back("H3");
    m_atm["METH"].push_back("H4");
    m_atm["METH"].push_back("H5");
    m_atm["METH"].push_back("H6");
    m_atm["NH3"].push_back("NA");
    m_atm["NH3"].push_back("HA");
    m_atm["NH3"].push_back("HA");
    m_atm["NH3"].push_back("HA");
    ...
    // Iterate through the coordinates of all the solutes present and check which molecule & corresponding atom is denoted by that particular coordinate and print in the output file.
    for (int i = 0 ; i < solutes_size ; i++)
    {
        if(i>=real_map[solute1] and (real_set.find(solute2) == real_set.end() or (real_set.find(solute2) != real_set.end() and  i<real_map[solute2] ))) // Check for solute1
        {
            temp_ct = (i - real_map[solute1]);
            temp_2 = molecule_atoms[solute1];
            temp_3 = (temp_2 != 0) ? m_atm[solute1][temp_ct%temp_2] : molecule_id[solute1];
            outFile << setprecision(3);
            outFile << setw(5)  << temp_3;
            outFile << setw(8) << solutes[i][0] * fact;
            outFile << setw(8) << solutes[i][1] * fact;
            outFile << setw(8) << solutes[i][2] * fact;
            outFile << "\n";
            atom_counter++;
        }
        else if(i>=real_map[solute2] and (real_set.find(solute3) == real_set.end() or (real_set.find(solute3) != real_set.end() and  i<real_map[solute3] ))) // Check for solute2
        {
            temp_ct = (i - real_map[solute2]);
            temp_2 = molecule_atoms[solute2];
            temp_3 = (temp_2 != 0) ? m_atm[solute2][temp_ct%temp_2] : molecule_id[solute2];
            outFile << setprecision(3);
            outFile << setw(5)  << temp_3;
            outFile << setw(8) << solutes[i][0] * fact;
            outFile << setw(8) << solutes[i][1] * fact;
        ...
        else if(i>=real_map[solute3] and (real_set.find(solute4) == real_set.end() or (real_set.find(solute4) != real_set.end() and  i<real_map[solute4] ))) // Check for solute3
        {
            temp_ct = (i - real_map[solute3]);
            temp_2 = molecule_atoms[solute3];
            temp_3 = (temp_2 != 0) ? m_atm[solute3][temp_ct%temp_2] : molecule_id[solute3];
            outFile << setprecision(3);
            outFile << setw(5) << temp_3 ;
            outFile << setw(8) << solutes[i][0] * fact;
            outFile << setw(8) << solutes[i][1] * fact;
        ...
        else if(i>=real_map[solute4]) // Check for solute4. Similarly can we extended for more than 4 solutes.
        {
            temp_ct = (i - real_map[solute4]);
            temp_2 = molecule_atoms[solute4];
            temp_3 = (temp_2 != 0) ? m_atm[solute4][temp_ct%temp_2] : molecule_id[solute4];
            outFile << setprecision(3);
            outFile << setw(5) << temp_3 ;
            outFile << setw(8) << solutes[i][0] * fact;
            outFile << setw(8) << solutes[i][1] * fact;
        ...
        for ( int j = 0 ; j < solutes.size() ; j++)
        {
            ...
            if (dist <= 0.2)        //If distance between COM of cage and any Methane molecule is less than cut-off, append it to gro file.
            {
                if(find(temp_meth_number_holder.begin(), temp_meth_number_holder.end(), j) == temp_meth_number_holder.end()) //If the value of "j" (methane index) does not exist in the temp_meth_number_holder, then add it to the vector and write its coordinates to gro file. Otherwise, do not write it in gro output file. The find(v.begin(),v.end(),"value") gives the iterator number of found value in the vector. If the value is not in the vector, iterator will be equal to v.end().
                {
                    ...
                    // outFile << "\n";
                    // atom_counter++;
                    if(j>=real_map[solute1] and (real_set.find(solute2) == real_set.end() or (real_set.find(solute2) != real_set.end() and  j<real_map[solute2] ))) // Check if entrapped molecule is solute1
                    {
                        temp_ct = (j - real_map[solute1]);  // temp_ct stores the atom serial number of a particular molecule
                        temp_2 = molecule_atoms[solute1];  // temp_2 stores the number of atoms in that particular molecule
                        temp_3 = (temp_2 != 0) ? m_atm[solute1][temp_ct%temp_2] : molecule_id[solute1]; // temp_3 stores the atom name
                        outFile << setprecision(3);
                        outFile << setw(5) << temp_3 << "X";
                        outFile << setw(7) << solutes[j][0] * fact;
                        outFile << setw(8) << solutes[j][1] * fact;
                        outFile << setw(8) << solutes[j][2] * fact;
                        outFile << "\n";
                        atom_counter++;
                    }
                    else if(j>=real_map[solute2] and (real_set.find(solute3) == real_set.end() or (real_set.find(solute3) != real_set.end() and  j<real_map[solute3] )))  // Check if entrapped molecule is solute2
                    {
                    	 temp_ct = (j - real_map[solute2]);
            		 temp_2 = molecule_atoms[solute2];
            		 temp_3 = (temp_2 != 0) ? m_atm[solute2][temp_ct%temp_2] : molecule_id[solute2];
                        outFile << setprecision(3);
                        outFile << setw(5) << temp_3 << "X" ;
                    ...
                    else if(j>=real_map[solute3] and (real_set.find(solute4) == real_set.end() or (real_set.find(solute4) != real_set.end() and  j<real_map[solute4] )))   // Check if entrapped molecule is solute3
                    {
                    	 temp_ct = (j - real_map[solute3]);
            		 temp_2 = molecule_atoms[solute3];
            		 temp_3 = (temp_2 != 0) ? m_atm[solute3][temp_ct%temp_2] : molecule_id[solute3];
                        outFile << setprecision(3);
                        outFile << setw(5) << temp_3 << "X";
                    ...
                    else if(j>=real_map[solute4])  // Check if entrapped molecule is solute4
                    {
                    	 temp_ct = (j - real_map[solute4]);
            		 temp_2 = molecule_atoms[solute4];
            		 temp_3 = (temp_2 != 0) ? m_atm[solute4][temp_ct%temp_2] : molecule_id[solute4];
                        outFile << setprecision(3);
                        outFile << setw(5) << temp_3 << "X";
    ...
    if(N_cage>0)
    {
        ...
        while ( !fin.eof() )
        {
            ...
            if(line_number==1)fout << atom_counter-1 << "\n\n"  ;
