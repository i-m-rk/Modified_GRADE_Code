...
//code modified between line number 353-495 and extra arguments are passed in the given function print_vmd_cage64512_frings
...
int main(int argc, char *argv[])
{
    ...
    // defined variable for 4 solute if we require more than 4 solute just add more variable here 
    map<string,int>mp,map_count,real_map;
    /* defined three different map
    mp-> it is storing number of solutes there in input file
    map_count-> counting the frequency of different solutes
    real_map-> it is same as mp will see later
    */ 
    ...
    temp1 = rawFilename + "_cage62512.xyz";
    temp2 = rawFilename + "_cage512.xyz";
    remove(temp1.c_str());
    remove(temp2.c_str());
    ...
    while (!fileIN.eof())
    {
        ...
        if (lineNumber == 1 || (lineNumber == (1 + 3 * frameCounter + frameCounter * Natoms) && !fileIN.eof())) // This is to find the first line of gro file.
        {
            ...
            if (ring5_temp.size() > 0)
                coplanar_Points(ring5_temp, atom_Pos, time, ring5, THETA); // Find the 5-rings which form a plane and get rid of the rest.
            ...
            if (ring6_temp.size() > 0)
                coplanar_Points(ring6_temp, atom_Pos, time, ring6, THETA);
            ...
            if (count_512_cups > 0)
            {
                ...
                if (cage_512_count > 0 && (frameCounter % DT == 0))
                    print_vmd_cage64512_frings(cup512, cage_512_count, cage_512_rings, ring5, ring6, atom_Pos, time, rawFilename, box_size_xyz, solutes, methane_512, topSolute, solute1, solute2,solute3, solute4, frameCounter,map_count,real_map,real_set);
            }
            ...
            if (count_62512_cups > 0)
            {
                ...
                if (cage_62512_count > 0 && (frameCounter % DT) == 0)
                    print_vmd_cage64512_frings(cup62512, cage_62512_count, cage_62512_rings, ring5, ring6, atom_Pos, time, rawFilename, box_size_xyz, solutes, methane_62512, topSolute, solute1, solute2,solute3, solute4, frameCounter,map_count,real_map,real_set);
            }
            ...
            print_vmd_cage64512_frings(cup62512, cage_64512_count, cage_64512_rings, ring5, ring6, atom_Pos, time, rawFilename, box_size_xyz, solutes, methane_64512, topSolute, solute1, solute2,solute3, solute4, frameCounter,map_count,real_map,real_set);
